...
def print_workflow_summary(result: dict[str, Any]) -> None:
    ...
    if final_report:
        ...
        print(f"  Ambiguity-uncovered:   {coverage_summary.get('ambiguity_uncovered')}")
...
def main() -> None:
    ...
    parser.add_argument("--vplan-file", required=True, help="Generated vPlan JSON.")
    parser.add_argument(
        "--edge-case-file", required=True, help="Generated edge-case JSON."
    )
    parser.add_argument(
        "--weak-words-file", required=True, help="Weak-language report JSON."
    )
