...
- Do not improve coverage because a supporting requirement is merely similar or shares a
  category.
...
- Never omit a linked test because it is uncovered, incomplete, or unsuitable; record it
...
- A linked row marked uncovered is traceability only and cannot count as suitable detail.
- A linked row with an empty description, no executable steps, or no expected results
  cannot count as suitable detail.
...
- Do not trust the vPlan coverage label by itself; judge the actual requirement-grounded
  description, actions, constraints, and expected results.
- When evidence is borderline, choose too_broad or unclear rather than suitable_detail.
"""
...
def calculate_granularity_adequacy(
    ...
    assessments_by_id: dict[str, GranularityAssessment] = {}
    ...
    for requirement in spec_requirements:
        ...
        if not linked_vplan_items:
            assessments_by_id[requirement_id] = GranularityAssessment(
                requirement_id=requirement_id,
                suitable_detail=False,
                granularity_label="not_mapped",
    ...
    model_assessments, usage = run_review_batches(
        review_name="granularity_adequacy",
