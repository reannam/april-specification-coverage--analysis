...
from Backend.Coverage.ambiguity_uncovered_coverage import (
    run_ambiguity_uncovered_coverage,
)
...
from Backend.config import (
    COVERAGE_STATUS_DIR,
    COVERAGE_WORKFLOW_IMAGE_PATH,
    COVERAGE_WORKFLOW_USAGE_SOURCE,
    FINAL_COVERAGE_DIR,
    GENERATE_WORKFLOW_IMAGES,
    LANGSMITH_LOGS_DIR as USAGE_LOGS_DIR,
)

from Backend.vPlan.post_processing.usage_logger import aggregate_usage, save_usage_log
from Backend.vPlan.post_processing.analyse_usage_logs import generate_usage_reports
...
def coverage_status_node(state: CoverageWorkflowState) -> dict[str, Any]:
    ...
    output_file = COVERAGE_STATUS_DIR / f"verified_coverage_status_{timestamp}.json"
    ...
    if isinstance(result, dict):
        ...
        coverage_status_result = result.get("coverage_status_result", {})
    ...
    if coverage_status_result:
        node_output["coverage_status_result"] = coverage_status_result
...
def ambiguity_uncovered_node(state: CoverageWorkflowState) -> dict[str, Any]:
    ...
    return {
        "ambiguity_uncovered_result": result,
    }
...
def final_report_node(state: CoverageWorkflowState) -> dict[str, Any]:
    ...
    ambiguity_uncovered_result = state.get("ambiguity_uncovered_result", {})

    if ambiguity_uncovered_result:
        coverage_percentages = dict(report.get("coverage_percentages", {}))
        coverage_percentages["ambiguity_uncovered_rate"] = float(
            ambiguity_uncovered_result.get("ambiguity_uncovered_rate", 0) or 0
        )
        report["coverage_percentages"] = coverage_percentages
        report["ambiguity_uncovered"] = ambiguity_uncovered_result
    ...
    if granularity_result:
        ...
        granularity_output_file = (
            FINAL_COVERAGE_DIR
            / f"granularity_adequacy_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
        )
    ...
    summary_file = output_files.get("coverage_summary_file")
    if summary_file:
        with Path(summary_file).open("w", encoding="utf-8") as file:
            json.dump(
                {
                    "metadata": report.get("metadata", {}),
                    "coverage_summary": report.get("coverage_summary", {}),
                    "coverage_percentages": report.get("coverage_percentages", {}),
                },
                file,
                indent=2,
                ensure_ascii=False,
    ...
    final_report_file = output_files.get("final_coverage_report_file")
    if final_report_file:
        with Path(final_report_file).open("w", encoding="utf-8") as file:
...
def usage_summary_node(state: CoverageWorkflowState) -> dict[str, Any]:
    ...
    save_usage_log(
        # This is a stable logical source label; the actual usage log is written
        # beneath USAGE_LOGS_DIR by save_usage_log.
        output_file=COVERAGE_WORKFLOW_USAGE_SOURCE,
...
def save_workflow_image(chain) -> None:
    # Graph rendering can require an external renderer, so cache the result.
    if COVERAGE_WORKFLOW_IMAGE_PATH.exists():
        return

    try:
        image_bytes = chain.get_graph().draw_mermaid_png()
        COVERAGE_WORKFLOW_IMAGE_PATH.write_bytes(image_bytes)
        print(f"Workflow image saved to {COVERAGE_WORKFLOW_IMAGE_PATH}")
...
def build_coverage_workflow():
    ...
    graph.add_node("ambiguity_uncovered", ambiguity_uncovered_node)
    ...
    graph.add_edge("coverage_status_verifier", "ambiguity_uncovered")
