...
from Backend.vPlan.agents.edge_case_agent import edge_case_agent_call
from Backend.vPlan.agents.vplan_category_agent import categorise_vplan
from Backend.vPlan.agents.vplan_generator_agent import v_plan_agent_call
from Backend.vPlan.agents.requirement_category_agent import categorise_requirements
from Backend.config import (
    GENERATE_WORKFLOW_IMAGES,
    LANGSMITH_LOGS_DIR as USAGE_LOGS_DIR,
    VPLAN_WORKFLOW_IMAGE_PATH,
)
...
from Backend.vPlan.pre_processing.data_class import GraphState
from Backend.vPlan.pre_processing.preprocess_requirements import (
    preprocess_requirements_file,
)
from Backend.vPlan.report_generation.uncovered_test_report_generator import (
    export_uncovered_test_report,
)
from Backend.vPlan.report_generation.traceability_record_generator import (
...
def requirement_category_node(state: GraphState) -> dict:
    started = perf_counter()
    result = categorise_requirements(state["requirements_file"])
    print("Requirement categorisation took " f"{perf_counter() - started:.2f} seconds.")
    return result
...
def usage_summary_node(state: GraphState) -> dict:
    ...
    requirement_category_usage = state.get("requirement_category_usage", {})

    summary = aggregate_usage(
        vplan_usage,
        edge_case_usage,
        category_usage,
        requirement_category_usage,
    )
    ...
    log_file = save_usage_log(
        ...
        trace_ids={
            ...
            "requirement_category_trace_ids": state.get(
                "requirement_category_trace_ids"
            ),
        },
...
def save_workflow_image(chain) -> None:
    ...
    if VPLAN_WORKFLOW_IMAGE_PATH.exists():
        return

    try:
        image_bytes = chain.get_graph().draw_mermaid_png()
        VPLAN_WORKFLOW_IMAGE_PATH.write_bytes(image_bytes)

        print(f"Workflow image saved to {VPLAN_WORKFLOW_IMAGE_PATH}")
...
def build_workflow():
    ...
    workflow.add_node(
        "categorise_requirements",
        requirement_category_node,
    )
    workflow.add_node(
        "vplan_generator",
        vplan_node,
    ...
    workflow.add_node(
        "uncovered_test_report",
        uncovered_test_report_node,
    )
    workflow.add_node(
        "usage_summary",
        usage_summary_node,
