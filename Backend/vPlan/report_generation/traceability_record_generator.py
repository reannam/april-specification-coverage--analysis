...
def build_requirement_test_map(
    vplan_rows: list[dict],
) -> dict[str, dict[str, list[str]]]:
    requirement_test_map = defaultdict(
        lambda: {"test_ids": [], "supporting_requirement_ids": []}
    )
    ...
    for row in vplan_rows:
        ...
        requirement_test_map[requirement_id]["test_ids"].append(test_id)
        requirement_test_map[requirement_id]["supporting_requirement_ids"].extend(
            str(item) for item in row.get("supporting_requirement_ids", []) if item
        )

    return {
        requirement_id: {
            "test_ids": values["test_ids"],
            "supporting_requirement_ids": sorted(
                set(values["supporting_requirement_ids"])
            ),
        }
        for requirement_id, values in requirement_test_map.items()
    }
...
def export_requirement_test_links(vplan_file: Path) -> Path:
    ...
    with csv_file.open("w", encoding="utf-8", newline="") as file:
        ...
        for requirement_id, related_tests in requirement_test_map.items():
            writer.writerow(
                {
