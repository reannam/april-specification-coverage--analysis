...
20. [Internal-inconsistency output](#20-internal-inconsistency-output)
21. [Extraction-quality output](#21-extraction-quality-output)
...
4. Review the complete-document extraction counts.
...
1. Open **Check for Inconsistencies** under **Analyse and compare**.
2. Upload one complete specification PDF.
3. Select **Check for inconsistencies**. By default, six independent reviewers inspect the PDF.
4. Wait for the reviewers and deterministic majority calculation to finish. Progress is printed in the backend terminal.
5. Review only the consensus records in the main results list. Expand **Independent reviewer outputs** when auditing why a finding did or did not reach consensus.
6. Download the consensus report for retention.

A finding is included only when at least `(reviewer count // 2) + 1` reviewers independently identify the same entity, category, pages, and sections. This is model-assisted evidence, not proof of a specification defect.
...
## 20. Internal-inconsistency output

Typical filenames are `inconsistency_consensus_<timestamp>.json` and `inconsistency_<timestamp>_reviewer_<number>.json` under `outputs/analysis_and_comparison/inconsistency_reports`.

### Why these files exist

- **Consensus report:** contains only contradictions that reached the deterministic majority threshold. This is the primary review output shown in the interface.
- **Reviewer outputs:** preserve every valid structured response so engineers can audit votes, rejected candidates, and differences between reviewers.

### Consensus report top-level values

| Value | Meaning |
| --- | --- |
| `metadata` | Run configuration, completion counts, failures, token usage, and consensus totals. |
| `inconsistencies` | Findings that reached the configured majority. An empty array means no candidate reached consensus, not that the specification is proven consistent. |

### `metadata` values

| Value | Meaning |
| --- | --- |
| `created_at` | UTC ISO timestamp when the consensus report was assembled. |
| `source_file` | Backend upload filename used for the run. It can contain a runtime prefix. |
| `model` | Model supplied to every independent reviewer. |
| `requested_reviewers` | Number of model reviews requested. The default is six. |
| `successful_reviewers` | Reviews that returned valid structured JSON. |
| `majority_threshold` | Minimum number of distinct reviewer votes required for inclusion. It is calculated as `(requested_reviewers // 2) + 1`. |
| `unique_candidate_findings` | Distinct normalised findings reported before majority filtering. |
| `consensus_findings` | Findings retained after majority filtering. |
| `failed_reviewers` | Array of reviewer numbers and error messages for unsuccessful calls. |
| `usage.input_tokens` | Total reported input tokens across successful reviewers. |
| `usage.output_tokens` | Total reported output tokens across successful reviewers. |
| `usage.total_tokens` | Total reported tokens across successful reviewers. |

### Inconsistency-record values

| Value | Meaning |
| --- | --- |
| `title` | Short reviewer-generated name for the contradiction. |
| `entity` | Signal, register, field, opcode, formula, encoding, state machine, or other object shared by both statements. |
| `category` | Type of contradiction, such as normative conflict, enumeration mismatch, or cross-reference error. Categories are descriptive labels, not scores. |
| `page_a`, `page_b` | PDF page numbers for the two statements, or `null` when the source cannot provide a page. The prompt requires citations, so null pages require particular scrutiny. |
| `section_a`, `section_b` | Section, table, or paragraph identifiers for both sides. |
| `statement_a`, `statement_b` | Short quotations or close paraphrases of the two allegedly conflicting statements. |
| `reason` | Explanation of why both statements cannot simultaneously be true. |
| `confidence` | Reviewer label of `Low`, `Medium`, or `High`. It is not a calibrated probability. |
| `agent_votes` | Number of distinct successful reviewers that reported the normalised finding. Duplicate records from one reviewer count as one vote. |

Consensus identity is based on the category, page pair, and section pair, with the two sides treated as order-independent. Similar paraphrases at the same cited locations can therefore vote together. Findings at different locations do not vote together.

## 21. Extraction-quality output
...
### Inconsistency checking cannot establish a majority

The run needs at least `majority_threshold` valid reviewer responses. Check the terminal for quota, model-access, file-upload, or structured-output failures. Confirm `OPENAI_API_KEY`, `INCONSISTENCY_MODEL`, and `INCONSISTENCY_AGENT_COUNT`, then rerun. Reducing the reviewer count lowers cost but also changes the evidence threshold.
...
- Internal-inconsistency checking uploads the complete PDF to the configured model provider and makes several full-document calls. It can be slow and expensive, depends on model/file limits, and can miss contradictions or produce false consensus.
- Majority voting measures agreement between model samples, not factual correctness. Review both cited source statements before accepting a finding.
